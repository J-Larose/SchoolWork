...
/*  Uncomment the following lines once addStudent is implemented  */
 ...
 addStudent(comp2401, joe);
  ...
  for (int i = 0; i < comp2401->count; i++)
  {
    free(comp2401->arr[i]);
  }
  free(comp2401->arr);
...
void growArray(StudentArray * stuArray) {
  StudentType **holdarr = (StudentType **) calloc(stuArray->capacity+5, sizeof(StudentType *));
  for (int i = 0; i < stuArray->count; ++i)
  {
    holdarr[i] = stuArray->arr[i];
  }

  free(stuArray->arr);
  stuArray->arr = holdarr;
  stuArray->capacity+=5;
}
...
void addStudent(StudentArray * stuArray, StudentType * stu) {
  if(stuArray->capacity <= stuArray->count){
    growArray(stuArray);
  }
